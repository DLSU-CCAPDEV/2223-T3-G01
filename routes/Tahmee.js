...
app.get('/edit/:id', async (req, res) => {
    let getTahmee = await Tahmee.findOne({_id:req.params.id});
    if(getTahmee.username != req.session.username){
        res.redirect('/Tahmee');     
    }
  
    res.render('edit2',{getCheese:getTahmee});
});

app.post('/edit/:id', async (req, res) => {
    let getTahmee = await Tahmee.findOne({_id:req.params.id});

       getTahmee.content = req.body.comment;

       getTahmee.save();
  
    res.render('edit2',{getTahmee:getTahmee});
});
...
app.post('/add/tahmee-review',async(req, res) => {
  const { replay, commentId } = req.body;
  let profile = await Profile.findOne({username:req.session.username });
  let tahmee = await Tahmee.findOne({_id:req.body.commentId });
    ...
    tahmee.reviews.push(data)


    tahmee.save().then(check =>{
    res.redirect('/Tahmee');
